...
#pragma comment( lib, "PhysFS/lib86/physfs.lib" )

#include "mmgr/mmgr.h"

using namespace std;

ModuleFileSystem::ModuleFileSystem(Application* app, const char* game_path, bool start_enabled) : Module(app, start_enabled)
{
	...
	const char* dirs[] = { ASSETS_FOLDER, LIBRARY_FOLDER, LIBRARY_TEXTURE_FOLDER, LIBRARY_MESH_FOLDER, LIBRARY_MODEL_FOLDER, LIBRARY_MATERIAL_FOLDER };
...
bool ModuleFileSystem::Init()
{
	...
	char* write_path = SDL_GetPrefPath(ORG, TITLE);
...
bool ModuleFileSystem::SaveUnique(string& name, const void * buffer, uint size, const char * path, const char * prefix, const char * extension)
{
	...
	sprintf_s(result, 250, "%s%s_%llu.%s", path, prefix, App->scene->GenerateRandomUUID(), extension);
